-- Combined Polygon, Solana, and Tron Wallet weekly Transfer Summary
...
SELECT *,
  SUM(amount) OVER (PARTITION BY blockchain ORDER BY week) AS total_chain_amt,
  SUM(amount_usd) OVER (PARTITION BY blockchain ORDER BY week) AS total_chain_amt_usd,
  SUM(amount) OVER (ORDER BY week) AS total_amt,
  SUM(amount_usd) OVER (ORDER BY week) AS total_amt_usd,
  SUM(transfers) OVER (PARTITION BY blockchain ORDER BY week) AS total_chain_tx,
  SUM(transfers) OVER (ORDER BY week) AS total_tx
FROM weekly_combined
ORDER BY week DESC, blockchain;